...
# Lab overlay: inject requestedAuth + individualIdType (UIN/VID) into IDA plugin after wget.
if [[ -d /opt/esignet-patches/io ]]; then
  echo "Applying IDA plugin class overlay from /opt/esignet-patches"
  if [[ -n "${plugin_zip_filename:-}" && -f "${plugins_path_env}/${plugin_zip_filename}" ]]; then
    java -cp /opt/esignet-patches ZipUpdater "${plugins_path_env}/${plugin_zip_filename}" /opt/esignet-patches
  fi
  mkdir -p "${plugins_path_env}/io"
  cp -a /opt/esignet-patches/io/. "${plugins_path_env}/io/"
fi

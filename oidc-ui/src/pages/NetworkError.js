...
const NetworkError = () => {
  ...
  const tryAgain = () => {
    ...
    window.location.replace(
      location.state?.path || process.env.PUBLIC_URL || '/'
    );
  };

  return (
    <div className="w-full py-16 sm:py-20 rounded-[6px] bg-white/10 backdrop-blur-2xl border border-white/30 ring-1 ring-white/20 shadow-[0_8px_30px_rgba(0,0,0,0.12)] flex flex-col items-center px-5 text-white">
      <svg
        width="102"
        height="102"
        viewBox="0 0 102 102"
      ...
      <p className="text-[1.5rem] font-semibold text-center mt-8 mb-2 text-white">
        {t('errors.network_error.header')}
      </p>
      <p className="text-white/80 text-center text-sm sm:text-base max-w-sm">
        {t('errors.network_error.subHeader')}
      </p>

      <button
        type="button"
        className="flex justify-center font-medium rounded-[6px] py-3 text-center mt-8 w-full max-w-xs m-auto bg-[#0F4356] text-white border-2 border-[#0F4356] hover:opacity-90 focus:outline-none focus:ring-2 focus:ring-white/40"
        onClick={tryAgain}
        id="try_again"
      >
